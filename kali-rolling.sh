...
(( STAGE++ )); echo -e "\n\n ${GREEN}[+]${RESET} (${STAGE}/${TOTAL}) Installing ${GREEN}gnomecalculator${RESET} ~ CLI subnet calculator"
apt -y -qq install gnome-calculator \
  || echo -e ' '${RED}'[!] Issue with apt install'${RESET} 1>&2
...
git clone -q -b master https://github.com/vrana/adminer.git /opt/adminer-git/ \
  || echo -e ' '${RED}'[!] Issue when git cloning'${RESET} 1>&2
pushd /opt/adminer-git/ >/dev/nullgraudit
git pull -q
php compile.php 2>/dev/nullgraudit
popd >/dev/null
#--- Link to others
apt -y -qq install webshells \
  || echo -e ' '${RED}'[!] Issue with apt install'${RESET} 1>&2
